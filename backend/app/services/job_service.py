...
def get_job_page(job_data):
    ...
    try:
        results = filter_page(results, page_size=job_data['page_size'], page=job_data['page'])
    except:
...
def post_job(job_data):     
    ...
    created_at = datetime.now(timezone("Asia/Ho_Chi_Minh")).strftime("%Y-%m-%d %H:%M:%S")
    ...
    # Add to database
    try:
        new_job = JobModel(job_name=job_name, job_description=job_description, created_at=created_at)
...
def update_job(job_data, job_id):
    ...
    try:   
        if job_exist.job_name != job_name or job_exist.job_description != job_description:
