...
def filter_candidate_matching(matching_data):   
    ...
    # Filter 
    results = filter_page(results, page_size=matching_data['page_size'], page=matching_data['page'], job_id=job_id)
    try:
        pass
    except:
...
def filter_page(results, page_size, page, job_id):
    ...
    modified_results = sorted(modified_results, key=lambda candidate: int(candidate['score']), reverse=True)  
    ...
    return {"results": paginated_results, "total_page": total_page, "total_matching": total_matching} 
