...
def get_list_candidate(file_data):
    ...
    try:
        results = filter_page(results, page_size=file_data['page_size'], page=file_data['page'])
    except:
...
def process_upload_file(single_file):
    try:
        ...
        # Add datetime
        time_upload = datetime.now(timezone("Asia/Ho_Chi_Minh")).strftime("%Y%m%d%H%M%S-")
    ...
    # Add to database
    try:
        candidate_name = output_analysis['name'] if output_analysis['name'] != "" else file_name_upload
        current_datetime = datetime.now(timezone("Asia/Ho_Chi_Minh")).strftime("%Y-%m-%d %H:%M:%S")
        file_upload_new = CandidateModel(candidate_name=candidate_name, 
                                         ...
                                         cv_hash=filehash, 
                                         cv_type=file_type, cv_size=filesize, cv_date=current_datetime)
