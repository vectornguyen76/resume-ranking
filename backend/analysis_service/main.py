...
class DocumentAnalyzer:
    ...
    def test_cv(self, file_path, req):
        ...
        json2file(output2json(self.gpt_analysis(self.get_cv(file_path), self.fn_cv_analysis)), config.default_json_resume_filename)
        logger.info(f'done: Resume: {file_path}')
        json2file(output2json(self.gpt_analysis(req, self.fn_cv_analysis)), config.default_json_req_filename)
        logger.info('done: Requirement')
        json2file(output2json(self.gpt_analysis(self.generate_content(), self.fn_matching_analysis)), config.default_json_score_filename)
    ...
    def analysis_cvs(self):
        ...
        for filename in file_list:
            ...
            json_save_path = os.path.join(self.output_path, self.json_filename(filename))
            
            if os.path.isfile(file_path):
                resume = self.get_cv(file_path)
                json2file(output2json(self.gpt_analysis(resume, self.fn_cv_analysis)), json_save_path)
    ...
    def analyse_candidate(self, file_name):
        ...
        json2file(data=json_output, file_dir=self.cv_analysis_dir, file_name=file_name_json)
        ...
        return {"name": json_output["PersonalInformation"]["name"],
    ...
    def analyse_job(self, job_name, job_description):
        ...
        json2file(data=json_output, file_dir=self.job_analysis_dir, file_name=file_name_json)
    ...
    def analyse_matching(self, cv_file_name, job_file_name):
        ...
        content = self.generate_content(cv_file_name=cv_file_name, job_file_name=job_file_name)
        # print(content)
        ...
        json2file(data=json_output, file_dir=self.matching_analysis_dir, file_name=matching_filename)
